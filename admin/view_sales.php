...
$sql="SELECT * FROM `orders` JOIN products JOIN users ON orders.productId=products.product_id AND orders.userId=users.id WHERE orderStatus='Delivered' ";
    ...
    <div class="sales" style="margin-top:20px;" id="sales">
        ...
        ?>
         <!-- order_id	productId	 -->
            ...
            $sql="SELECT *, SUM(products.price) as priceTotal FROM `orders` JOIN products ON orders.productId=products.product_id   WHERE `orderStatus`='Delivered'";
            $stmt=$pdo->prepare($sql);
            $stmt->execute();
